...
import { Form, Input, InputNumber,Drawer, Popconfirm, Table } from 'antd';
...
const UsersPage=()=>{
    ...
    const [sideInfo,setSideInfo]=useState({fullName:'',key:'',email:'',phoneNumber:'',position:'',password:''})
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (_, record) => {
            return ( 
              <img src='https://www.vhv.rs/dpng/d/509-5097789_oic-provincial-statistics-officer-psa-maguindanao-profile-avatar.png' alt='profile' className='p-0 w-8 h-8 rounded-full'/>
            );
          },
      },
      {
    ...
    return (
      ...
      >
        ...
        <HiOutlineChevronDoubleRight  onClick={() => {
          setSideInfo({fullName:'',key:'',email:'',phoneNumber:'',position:'',password:''})
          setOpen(false)
          setInput(!Input)}}/>
        <IoResizeOutline />
        </div>
        ...
        <form action="" className='grid grid-cols-1 md:grid-cols-2 items-center gap-2'>
            ...
            {Input?<input type="text" id="fullName" name="fullName" value={sideInfo.fullName ||""} onChange={handleInput} placeholder='Enter full name' className='w-full p-2 rounded border focus:outline-none'></input>:<p className='hover:bg-zinc-100 py-1 px-2 rounded' onClick={()=>setInput(true)}>{sideInfo?.fullName}</p>}
            <p className='text-md font-bold px-2'>Position</p>
            {Input?<input type="text" id="position" name="position" value={sideInfo.position ||""} onChange={handleInput} placeholder='Enter position' className='w-full p-2 rounded border focus:outline-none'></input>:<p className='hover:bg-zinc-100 py-1 px-2 rounded' onClick={()=>setInput(true)}>{sideInfo?.position}</p>}
            <p className='text-md font-bold px-2'>Email</p>
            {Input?<input type="email" id="email" name="email" value={sideInfo.email ||""} onChange={handleInput} placeholder='Enter email' className='w-full p-2 rounded border focus:outline-none'></input>:<p className='hover:bg-zinc-100 py-1 px-2 rounded' onClick={()=>setInput(true)}>{sideInfo?.email}</p>}
            <p className='text-md font-bold px-2'>Phone number</p>
            {Input?<input type="tel" id="phoneNumber" name="phoneNumber" value={sideInfo.phoneNumber ||""} onChange={handleInput} placeholder='Enter phone number' className='w-full p-2 rounded border focus:outline-none'></input>:<p className='hover:bg-zinc-100 py-1 px-2 rounded' onClick={()=>setInput(true)}>{sideInfo?.phoneNumber}</p>}
            <p className='text-md font-bold px-2'>Password</p>
            {Input?<input type="text" id="password" name="password" value={sideInfo.password ||""} onChange={handleInput} placeholder='Add password' className='w-full p-2 rounded border focus:outline-none'></input>:<p className='hover:bg-zinc-100 py-1 px-2 rounded' onClick={()=>setInput(true)}>{sideInfo?.password}</p>}
            <button type='button' onClick={()=>updateRow(sideInfo.key)} className='p-2 rounded bg-blue-400 w-fit text-white col-span-full'>save</button>
        </form>
        </div>
      </Drawer>
